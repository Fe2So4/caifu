    ...
    <div>
        <div slot="header" class="breadcrumb-header">
            ...
            <el-breadcrumb separator-class="el-icon-arrow-right" style="line-height: 1.6">
                ...
                <el-breadcrumb-item>T+1车险统计</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <el-dialog :visible.sync="viewVisible" :close-on-click-modal="false" :close-on-press-escape="false"
            title="T+1车险保单详情" center>
            <tcar-view :policy="modelChosenToView"></tcar-view>
        </el-dialog>
    </div>
</template>
...
import tcarView from './TCarView.vue'
...
export default {
    ...
    data () {
        return {
            ...
            viewVisible: false,
            condition: {
                pageSize : this.$store.state.tcar.policys.pagesize,
                currentPage : this.$store.state.tcar.policys.current,
                orgCodes: [],
                isCommissions: [],
            },
            orgOptions : [], //可选机构
    ...
    mounted () {
        this.$store.dispatch('getTcarAuth')
    },
    methods : {
        ...
        handleSizeChange(val) {
            ...
            this.$store.dispatch("queryTCar", this.condition)
        },
        ...
        downloadExcel () {
            ...
            window.open("policy-t1/excel-car?"+obj2Params(params))
        },
    ...
    computed: {
        policys() {
            return this.$store.state.tcar.policys
        },
        urls () {
        ...
        authorities() {
            return this.$store.state.tcar.authorities
        }
    },
}
</script>