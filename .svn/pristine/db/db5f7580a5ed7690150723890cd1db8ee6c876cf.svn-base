    ...
    <div>
        <div slot="header" class="breadcrumb-header">
            ...
            <el-breadcrumb separator-class="el-icon-arrow-right" style="line-height: 1.6">
                ...
                <el-breadcrumb-item>T+1非车险统计</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
            ...
            <div style="margin-top: 30px; margin-bottom: 20px;">
                <el-table :data="policys.processedData" style="width: 100%" height="250px"
                    ...
                    <el-table-column v-for="(title, index) in policys.titles" :key="index" :label="title" 
                        :width="(title=='机构' || title=='省分机构' || title=='产品名称' || title=='保单号' || title=='投保单号')? 180 : 100">
        ...
        <el-dialog :visible.sync="viewVisible" :close-on-click-modal="false" :close-on-press-escape="false"
            title="T+1非车险保单详情" center>
            <tproperty-view :policy="modelChosenToView"></tproperty-view>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    data () {
        return {
            ...
            viewVisible: false,
            condition: {
                pageSize : this.$store.state.tproperty.policys.pagesize,
                currentPage : this.$store.state.tproperty.policys.current,
                orgCodes: [],
                isCommissions: [],
            },
            orgOptions : [], //可选机构
    ...
    created () {
        // this.$store.dispatch("refreshQueryInfo", this.$route.query)
    },
    mounted () {
        this.$store.dispatch('getTpropertyAuth')
    },
    methods : {
        ...
        handleSizeChange(val) {
            ...
            this.$store.dispatch("queryTProperty", this.condition)
        },
        ...
        downloadExcel () {
            ...
            window.open("policy-t1/excel-no-car?"+obj2Params(params))
        },
    ...
    computed: {
        policys() {
            return this.$store.state.tproperty.policys
        },
        urls () {
        ...
        authorities() {
            return this.$store.state.tproperty.authorities
        }
    },
}
</script>